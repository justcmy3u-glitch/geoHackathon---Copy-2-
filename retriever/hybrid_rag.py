...
class HybridRAG:
    def __init__(
        ...
        """All dependencies are optional and lazily initialised if not provided."""
        self.bm25 = bm25_index or BM25Index()
        self.qdrant = qdrant_writer or QdrantWriter(
            collection_name=os.environ.get("QDRANT_COLLECTION", "geo_documents")
        )
    ...
    def retrieve(self, query: str, top_k: int = 5, k: int = 60) -> list[dict]:
        ...
        for chunk_id, score in sorted_chunks[:top_k]:
            meta = docs[chunk_id].copy()
            meta["rrf_score"] = score
            # Ensure source prefix in content
            doc_id = meta.get("doc_id", "unknown")
            page_num = meta.get("page_num", 0)
            if not meta.get("content", "").startswith("[Источник:"):
                meta["content"] = f"[Источник: {doc_id}, Страница: {page_num}]\n{meta.get('content', '')}"
            results.append(meta)

        return results

    def search(self, query: str, top_k: int = 5) -> list[dict]:
        """Alias for retrieve() — used in tests."""
